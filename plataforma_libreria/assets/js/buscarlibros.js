function buscaLibros(pag=1) {
	//alert(pag)

	var url = 'servicios/buscalibros.php'

	//recuperar la palabra a buscar
	var palabra = document.getElementById('palabro').value.trim()
	...
	.then(function(libros) {
		...
		if (listaLibros.length==0) {
			...
			for (e = 1; e <= paginas; e++) {
				if (e == pag) {
					enlaces += `<input type='button' value='${e} ' onclick='buscaLibros(${e})' class='seleccionado'>`
				} else {
					enlaces += `<input type='button' value='${e} ' onclick='buscaLibros(${e})'>`
				}	
			}
